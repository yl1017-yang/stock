...
- **수집 파이프라인 (2-Track)**:
  ...
  2. **[AUTO] 국내 우량 가치주 저평가 발굴**:
     - **시장 전수 조사**: 특정 테마에 국한되지 않고 코스피/코스닥 전 종목(약 2,500개)을 대상으로 탐색을 시작합니다.
     - **선제적 필터링**: 1차적으로 Pandas를 이용해 재무 건전성(PER < 20, PBR < 2.0, 시가총액 > 1,000억)을 만족하는 종목을 선별합니다.
     - **정밀 검증**: 선별된 종목 중 120일 이평선이 역배열에서 정배열로 전환(Turnaround)되면서, 최근 분기 실적이 우상향하는 종목을 정밀 판별합니다.
     - **테마 매칭**: 발굴된 종목이 기존 성장 테마에 속해 있다면 해당 테마명을 병기하여 제공합니다.
  3. **[AUTO] 미국 성장주 저평가 발굴**:
     ...
     - **추세 분석**: 미국 종목 역시 **최근 1개월 수익률**을 함께 제공합니다.
- **투자 지표/수익성 검증**:
...
3.  **가이드 자동 업데이트**: 새로운 기능 추가나 로직 수정 시, **반드시 이 `GUIDE.md` 파일에도 해당 내용을 업데이트**해야 합니다. (사용자와 AI의 정렬 유지)
4.  **한국어 우선 원칙**: 프로젝트 내의 모든 문서(`.md` 파일) 및 사용자 가이드는 **반드시 한국어로 작성**해야 합니다.
...
**DART API 키**: 기업 실적을 조회하려면 [Open DART](https://opendart.fss.or.kr/)에서 발급받은 API 키가 필요합니다. 키가 준비되시면 깃허브 저장소의 **Settings > Secrets and variables > Actions**에 `DART_API_KEY`라는 이름으로 등록해 주셔야 합니다.

인베스팅 닷컴 : https://kr.investing.com/equities/most-undervalued
